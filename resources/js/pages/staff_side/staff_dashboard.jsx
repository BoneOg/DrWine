                ...
                <main className={`flex-1 px-4 sm:px-6 md:px-8 pt-16 md:pt-20 transition-all duration-300 ease-in-out ${isMobile ? 'ml-0' : 'ml-64'}`}>
                    ...
                    <div className="grid grid-cols-1 font-monts md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                        ...
                        <StatsCard
                            ...
                            value={totalPendingReservations !== undefined ? totalPendingReservations : 0}
                            color="yellow"
                        />
                        <StatsCard
                            title="Confirmed Bookings"
                            value={totalConfirmedReservations !== undefined ? totalConfirmedReservations : 0}
                            color="green"
                        />
                        <StatsCard
                            title="Cancelled Bookings"
                            value={totalCancelledReservations !== undefined ? totalCancelledReservations : 0}
                            color="red"
                        />
                        <StatsCard
                            title="Completed Bookings"
                            value={totalCompletedReservations !== undefined ? totalCompletedReservations : 0}
                            color="blue"
                        />
                    </div>
                    ...
                    <motion.div className="grid grid-cols-1 font-monts lg:grid-cols-2 gap-6 mb-8" {...fadeIn}>
                        ...
                        <div className="backdrop-blur-xl bg-white/[0.02] border border-white/10 p-6 rounded-lg">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <StatItem label="Total Users" value={userStats?.total !== undefined ? userStats.total : 0} />
                                <StatItem label="Regular Users" value={userStats?.user !== undefined ? userStats.user : 0} />
                                <StatItem label="New This Month" value={userStats?.newThisMonth !== undefined ? userStats.newThisMonth : 0} />
                            </div>
                        </div>
                        ...
                        <div className="backdrop-blur-xl bg-white/[0.02] border border-white/10 p-6 rounded-lg">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <StatItem
                                    ...
                                    value={`$${revenueStats?.today?.toLocaleString() || '0.00'}`}
                                />
                                <StatItem
                                    label="This Week"
                                    value={`$${revenueStats?.thisWeek?.toLocaleString() || '0.00'}`}
                                />
                                <StatItem
                                    label="This Month"
                                    value={`$${revenueStats?.thisMonth?.toLocaleString() || '0.00'}`}
                                />
                                <StatItem
                                    label="Total Revenue"
                                    value={`$${revenueStats?.total?.toLocaleString() || '0.00'}`}
                                />
                            </div>
                        </div>
                    </motion.div>
                                        ...
                                        >
                                            ...
                                            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
                                                ...
                                                <p className="text-right text-sm text-white/70">
                                                    ${activity.amount?.toLocaleString() || '0.00'}
                                                </p>
                                            </div>
                                        </div>
                                    ))
